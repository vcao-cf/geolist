...
export default function Home(){
 ...
 const[radiusData,setRadiusData]=useState<RadiusData|null>(null),[radiusCity,setRadiusCity]=useState(""),[radiusState,setRadiusState]=useState(""),[radiusMiles,setRadiusMiles]=useState<number|"">(""),[pickedCenter,setPickedCenter]=useState<RadiusPlace|null>(null);
 ...
 const radiusResults=useMemo(()=>!center||!radiusData||radiusMiles===""?[]:radiusData.zctas.map(z=>({zip:z.zip,distance:miles(center,z)})).filter(z=>z.distance<=radiusMiles).sort((a,b)=>a.distance-b.distance),[center,radiusData,radiusMiles]);
 ...
 mode==="radius"?<section className="workspace radius-workspace"><div className="radius-card card"><div className="title"><span><label>Nationwide radius</label><h2>Find ZIPs around a city</h2></span><mark>{radiusData?"2025 data loaded":"Loading data..."}</mark></div><p className="radius-intro">Choose a Census place and distance. Results include ZCTAs whose representative center point falls inside the radius.</p><div className="radius-form"><label><span>City</span><input value={radiusCity} onChange={e=>{setRadiusCity(e.target.value);setPickedCenter(null)}} placeholder="Enter a city"/></label><label><span>State</span><select value={radiusState} onChange={e=>{setRadiusState(e.target.value);setPickedCenter(null)}}><option value="" disabled hidden>Select a state</option>{STATES.map(([a,n])=><option key={a} value={a}>{a} - {n}</option>)}</select></label><label><span>Radius</span><select value={radiusMiles} onChange={e=>setRadiusMiles(Number(e.target.value))}><option value="" disabled hidden>Select a radius</option>{[5,10,15,25,50,75,100].map(n=><option key={n} value={n}>{n} miles</option>)}</select></label></div>{!center&&radiusCity.length>=2&&<div className="suggestions">{!radiusState?<p>Choose a state to search.</p>:suggestions.length?suggestions.map((p,i)=><button key={p.state+p.label+i} onClick={()=>{setRadiusCity(p.city);setPickedCenter(p)}}><b>{p.city}, {p.state}</b><small>{p.label}</small></button>):<p>No matching Census place in {radiusState}. Check the spelling or state.</p>}</div>}{center&&<><div className="radius-status"><b>{"\u2713"}</b><span><strong>{center.city}, {center.state}</strong><small>{radiusMiles===""?"Choose a radius to see results.":`${radiusResults.length} ZCTAs have a center point within ${radiusMiles} miles`}</small></span></div>{radiusMiles!==""&&<div className="nearby"><span>Nearest results</span>{radiusResults.slice(0,8).map(r=><b key={r.zip}>{r.zip}<small>{r.distance.toFixed(1)} mi</small></b>)}</div>}</>}<div className="method-note"><b>How radius works</b><p>This is a center-point test using official Census Gazetteer coordinates - not a drive-time area or exact ZIP-boundary intersection. ZCTAs approximate USPS delivery ZIPs, so review edge cases for high-value campaigns.</p></div></div>{outputPanel}</section>:
