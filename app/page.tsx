...
export default function Home(){
 ...
 const[radiusData,setRadiusData]=useState<RadiusData|null>(null),[radiusCity,setRadiusCity]=useState("Memphis"),[radiusState,setRadiusState]=useState("TN"),[radiusMiles,setRadiusMiles]=useState(25),[pickedCenter,setPickedCenter]=useState<RadiusPlace|null>(null);
 const[selectedStates,setSelectedStates]=useState<string[]>([]),[selectedCounties,setSelectedCounties]=useState<string[]>([]),[browseState,setBrowseState]=useState(""),[format,setFormat]=useState<"comma"|"line">("comma"),[copied,setCopied]=useState(false),[query,setQuery]=useState(""),[inspectQuery,setInspectQuery]=useState("");
 useEffect(()=>{fetch(dataUrl("geo-index.json")).then(r=>{if(!r.ok)throw Error();return r.json()}).then((data:GeoData)=>{if(Array.isArray(data.rows)&&data.rows.length){setPlaces(data.rows.map(([zip,state,stateName,county,city])=>({state,stateName,county,city,zips:[zip],aliases:[city]})));setDataSource(`${data.meta?.source??"SimpleMaps"} - ${data.meta?.release??"Basic"}`)}}).catch(()=>{});fetch(dataUrl("radius-index.json")).then(r=>r.json()).then((d:RadiusData)=>{setRadiusData(d);const center=d.places.find(p=>p.state==="TN"&&norm(p.city)==="memphis");if(center)setPickedCenter(center)}).catch(()=>{});},[]);
 ...
 const visible=useMemo(()=>STATES.filter(([a,n])=>(a+" "+n).toLowerCase().includes(query.toLowerCase())),[query]);
 ...
 <section className="inspect card"><div><label>Reverse lookup</label><h2>See what&apos;s inside a ZIP list.</h2><p>Paste ZIPs to identify their state, county, and city. Unknown and malformed entries are flagged instead of dropped.</p><textarea value={inspectInput} onChange={e=>setInspectInput(e.target.value)} placeholder="07601, 38110, 90001"/></div><div className="results">{!!reverse.length&&<div className="search"><input value={inspectQuery} onChange={e=>setInspectQuery(e.target.value)} placeholder="Filter by ZIP, city, county, or state"/></div>}{!!reverse.length&&<div className="inspect-meta"><b>{inspectHasFilter?`${inspectFiltered.length.toLocaleString()} of ${reverse.length.toLocaleString()} match`:`${reverse.length.toLocaleString()} unique entries`}</b><span>{inspectFiltered.length>inspectLimit?`Showing the first ${inspectLimit.toLocaleString()}. ${inspectHasFilter?"Narrow your filter":"Split the list"} to review the remainder.`:inspectHasFilter&&!inspectFiltered.length?"No matches for this filter.":"All entries shown"}</span></div>}<div className="thead"><b>ZIP</b><b>City</b><b>County</b><b>State</b></div>{displayedReverse.length?displayedReverse.map(({zip,places})=>{const found=places.length>0;return <div className={"row "+(!found?"unknown":"")} key={zip}><b>{zip}</b>{found?<><span>{unique(places.map(p=>p.city)).join(", ")}</span><span>{unique(places.map(p=>p.county)).join(", ")}</span><span>{unique(places.map(p=>p.state)).join(", ")}</span></>:<span className="notfound">{/^\d{5}$/.test(zip)?"Not found in loaded ZIP data":"Use a 5-digit ZIP"}</span>}</div>}):<div className="empty">{reverse.length?"No entries match this filter.":"Paste a ZIP list to inspect it."}</div>}</div></section>}
